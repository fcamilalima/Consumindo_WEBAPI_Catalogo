...
public class AcessaAPIService
{
        ...
        using (var client = new HttpClient())
        {
            ...
            using (var response = await client.GetAsync($"{URI}"))
            {
                if (response.IsSuccessStatusCode)
                {
                    ...
                    List<Produto> produtos = JsonSerializer.Deserialize<List<Produto>>(ProdutoJsonString);
                    return produtos;
                }
                else
                {
    ...
    public async Task<Produto> GetProdutoById(string URI, AccessToken accessToken)
    {
        using (var client = new HttpClient())
        {
            ...
            {
                var ProdutoJsonString = await response.Content.ReadAsStringAsync();
                Produto produto = JsonSerializer.Deserialize<Produto>(ProdutoJsonString);
                return produto;
            }
            else
            {
    ...
    public async Task<string> AddProduto(string URI, AccessToken accessToken, Produto produto)
    {
        using (var client = new HttpClient())
        {
            ...
            var content = new StringContent(
                serializedProduto, Encoding.UTF8, "application/json");
    ...
    public async Task<string> UpdateProduto(string URI, AccessToken accessToken, Produto produto)
    {
        using (var client = new HttpClient())
        {
            ...
            {
                return "Falha ao atualizar produto " + responseMessage.StatusCode;
            }
        }
    }
